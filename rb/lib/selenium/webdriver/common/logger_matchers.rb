  ...
  RSpec::Matchers.define :"have_#{level}" do |expected_ids, logger = nil|
    ...
    failure_message do
      but_message = if @found_ids.nil? || @found_ids.empty?
                      "not all [#{expected_ids}] entries were reported"
                    else
                      "instead these entries were found: [#{@found_ids.join(', ')}]"
                    end
      "expected #{expected_ids} to have been logged, but #{but_message}"
    end

    failure_message_when_negated do
      but_message = "it was found among these entries: [#{@found_ids.join(', ')}]"
